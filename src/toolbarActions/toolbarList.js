...
const toolbarList = (state) => {
    const curG = state.graphs[state.curGraphIndex] && state.graphs[state.curGraphIndex].instance;
    return [
        {
            type: 'action',
            text: 'Node',
        ...
        },
        {
            ...
            icon: FaSave,
            action: (s, d) => [
                { fn: () => curG && curG.pushToServer(), name: 'Save on Server' },
        ...
        { type: 'vsep' },
        {
            ...
            text: 'Push',
            icon: FiChevronUp,
            action: () => curG && curG.pushToServer(),
            active: curG && state.isWorkflowOnServer,
        },
        {
            type: 'action',
            text: 'Force Push',
            icon: FiChevronsUp,
            action: () => curG && curG.forcePushToServer(),
            active: curG && state.isWorkflowOnServer,
        },
        {
            type: 'action',
            text: 'Pull',
            icon: FiChevronDown,
            action: () => curG && curG.pullFromServer(),
            active: curG && state.isWorkflowOnServer,
        },
        {
            type: 'action',
            text: 'Force Pull',
            icon: FiChevronsDown,
            action: () => curG && curG.forcePullFromServer(),
            active: curG && state.isWorkflowOnServer,
        },
        { type: 'vsep' },
        { type: 'space' },
